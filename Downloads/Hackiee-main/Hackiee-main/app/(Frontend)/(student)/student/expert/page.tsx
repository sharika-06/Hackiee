...
import { Code, Shield, Send, Fingerprint, BookOpen, ChevronRight, Lock, Star, ArrowLeft, Brain } from 'lucide-react';
import SkillChart from '@/components/student/SkillChart';
import InterrogationModal from '@/components/student/InterrogationModal';
import { EXPERT_QUESTIONS, Question } from '@/lib/questions';
...
export default function ExpertDashboard() {
    ...
    const [currentQuestionId, setCurrentQuestionId] = useState(EXPERT_QUESTIONS[0].id);
    const [language, setLanguage] = useState<keyof Question['templates']>('javascript');
    const [keystrokes, setKeystrokes] = useState(0);

    const currentQuestion = EXPERT_QUESTIONS.find(q => q.id === currentQuestionId) || EXPERT_QUESTIONS[0];
            ...
            <div className="min-h-[calc(100vh-73px)] bg-[#050505] p-8 lg:p-12 overflow-y-auto">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="mb-12 text-center">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-black uppercase tracking-widest mb-4">
                            <Brain size={12} /> Expert · DSA Mastery
                        </div>
                        <h1 className="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500 mb-4">
                            DSA Challenge Stages
                        </h1>
                        <p className="text-gray-400 text-lg max-w-2xl mx-auto">
                            Advanced data structures and algorithms. These challenges measure your depth of computer science knowledge.
                        </p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {EXPERT_QUESTIONS.map((q, index) => {
                            const levelData = levels[q.id];
                            const isLocked = index > 0 && !levels[EXPERT_QUESTIONS[index - 1].id]?.isCompleted;
                                ...
                                <button
                                    ...
                                    className={`relative p-8 rounded-3xl text-left transition-all duration-300 group ${isLocked
                                        ...
                                        : isCompleted
                                            ...
                                            : 'bg-purple-500/10 border border-purple-500/20 hover:bg-purple-500/20 shadow-[0_0_30px_rgba(168,85,247,0.1)]'
                                        }`}
                                >
                                    {isLocked && (
                                    ...
                                    <div className="relative z-0">
                                        <div className="flex justify-between items-start mb-4">
                                            <div className={`text-[10px] font-black uppercase tracking-[0.2em] px-3 py-1 rounded-full ${isLocked ? 'bg-white/5 text-gray-500' : isCompleted ? 'bg-green-500/20 text-green-400' : 'bg-purple-500/20 text-purple-400'}`}>
                                                Stage {index + 1}
                                            </div>
                                            {isCompleted && (
                                        ...
                                        {isCompleted && (
                                            <div className="mt-6 flex items-center justify-between border-t border-white/5 pt-4">
                                                <span className="text-xs text-green-400/70 font-bold uppercase">Mastery Level</span>
                                                <span className="text-lg font-black text-white">{levelData.score}%</span>
                                            </div>
                                        )}
                                        {!isCompleted && !isLocked && (
                                            <div className="mt-6 flex items-center text-purple-400 font-bold text-sm gap-2 group-hover:translate-x-1 transition-transform">
            ...
            <aside className="w-80 border-r border-white/5 bg-[#0a0a0a]/50 flex-col hidden lg:flex">
                <div className="p-6 border-b border-white/5 pb-4">
                    ...
                    <h2 className="text-sm font-black uppercase tracking-widest text-gray-400 flex items-center gap-2">
                        <BookOpen size={16} className="text-purple-400" /> Expert DSA
                    </h2>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {EXPERT_QUESTIONS.map((q, idx) => {
                        const isLocked = idx > 0 && !levels[EXPERT_QUESTIONS[idx - 1].id]?.isCompleted;
                        return (
                            <button key={q.id}
                                disabled={isLocked}
                                ...
                                <p className={`text-sm font-bold truncate ${currentQuestionId === q.id ? 'text-purple-400' : 'text-gray-300'}`}>{q.title}</p>
                                {isLocked
            ...
            <div className="flex-1 flex flex-col gap-6 p-6 overflow-hidden">
                <div className="flex justify-between items-end shrink-0">
                    ...
                    <button onClick={() => { setShowModal(true); setInterrogationPending(true); }}
                        className="bg-gradient-to-r from-purple-500 to-pink-600 hover:opacity-90 transition-all px-8 py-3 rounded-xl font-bold flex items-center gap-3 shadow-lg shadow-purple-500/20 group shrink-0 text-white">
                        ...
                        <div className="bg-white/5 px-6 py-4 border-b border-white/10 flex items-center justify-between shrink-0">
                            <div className="flex items-center gap-6">
                                <div className="flex items-center gap-3 text-sm text-gray-300">
                                    <div className="p-1.5 rounded-lg bg-purple-500/10 border border-purple-500/20"><Code size={16} className="text-purple-400" /></div>
                                ...
                                <div className="flex items-center gap-1.5 p-1 bg-white/[0.03] rounded-xl border border-white/5">
                                    {LANGUAGES.map(lang => (
                                        <button key={lang.id} onClick={() => setLanguage(lang.id as keyof Question['templates'])}
                                            className={`px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${language === lang.id ? 'bg-purple-500 text-white' : 'text-gray-500 hover:text-gray-300'}`}>
                                            {lang.name}
                                        </button>
                                    ))}
                                </div>
                    ...
                    <div className="space-y-6 overflow-y-auto pr-1">
                        <div className="glass p-6 rounded-3xl border border-white/10">
                            <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                                <Fingerprint size={14} className="text-purple-400" /> Neural Skill Analytics
                            </h3>
                            <SkillChart data={{
                        ...
                        <div className="glass p-6 rounded-3xl border border-white/10">
                            <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                                <Shield size={14} className="text-purple-400" /> Integrity Metrics
                            </h3>
                        ...
                        <div className="glass p-6 rounded-3xl border border-white/10 bg-purple-500/5">
                            <h3 className="text-[10px] font-black text-purple-400 uppercase tracking-[0.2em] mb-3">Security Protocol</h3>
                            <p className="text-xs text-gray-500 leading-relaxed">VeriSkill AI is active. Analyzing behavioral patterns, environment integrity, and neural logic consistency.</p>
                        </div>
                    </div>
                </div>
            </div>
