...
export default function InterrogationModal({
    ...
    const [isAIVoiceDetected, setIsAIVoiceDetected] = useState(false);
    const [voiceCheckResult, setVoiceCheckResult] = useState<{ reason: string; confidence: number } | null>(null);
    const recognitionRef = useRef<any>(null);
    const modalRef = useRef<HTMLDivElement>(null);

    const currentQuestion = QUESTIONS.find(q => q.id === questionId) || QUESTIONS[0];

    const checkVoiceForAI = async (transcript: string) => {
        if (!transcript || transcript.trim().length < 10) return;
        setIsCheckingVoice(true);
        setIsAIVoiceDetected(false);
        setVoiceCheckResult(null);
        try {
            const res = await fetch('/api/verify-voice', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ transcript, code: submittedCode, questionId })
            });
            const data = await res.json();
            if (data.isAI && data.confidence > 60) {
                setIsAIVoiceDetected(true);
                setVoiceCheckResult({ reason: data.reason, confidence: data.confidence });
            } else {
                setIsAIVoiceDetected(false);
                setVoiceCheckResult(null);
            }
    ...
    const toggleVoiceRecording = () => {
        ...
        recognition.onresult = (event: any) => {
            ...
            if (finalTranscript) {
                collectedTranscript += (collectedTranscript.length > 0 ? ' ' : '') + finalTranscript;
        ...
        recognition.onerror = (event: any) => {
            ...
            if (event.error === 'network') {
                alert("Network error: Speech recognition requires an active internet connection or HTTPS context.");
            }
            setIsRecording(false);
        };
        ...
        recognition.onend = () => {
            setIsRecording(false);
            // When recording ends, check if the voice sounds AI-generated
            if (collectedTranscript.trim().length > 10) {
                checkVoiceForAI(collectedTranscript);
            }
        };
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center gap-4">
                                    <button
                                        ...
                                        className={`flex-1 border rounded-xl py-3 flex items-center justify-center gap-2 text-sm font-semibold transition-all ${isRecording
                                    ...
                                    >
                                        <Mic size={18} className={isRecording ? "text-red-400" : isCheckingVoice ? "text-yellow-400 animate-spin" : "text-blue-400"} />
                                        {isRecording ? 'Stop Recording' : isCheckingVoice ? 'Analyzing Voice...' : 'Use Voice Explanation'}
                                    </button>
                                    <button
                                        onClick={async () => {
                                            if (isVerifying || isAIVoiceDetected || isCheckingVoice) return;
                                            ...
                                            try {
                                                ...
                                                if (res.ok) {
                                                    ...
                                                    if (finalScore >= 50) {
                                                        ...
                                                        // Failed either code or integrity check - mark as completed without next unlock
                                                        completeLevel(questionId, null, finalScore, stars);
                                                    }

                                                    // Flag result in backend if AI marks wrong answers or integrity issues
                                                    const studentEmail = typeof window !== 'undefined' ? localStorage.getItem('userEmail') : null;
                                                    const studentName = typeof window !== 'undefined' ? localStorage.getItem('studentName') : null;
                                                    const examId = typeof window !== 'undefined' ? localStorage.getItem('examId') : 'HACKATHON_2026';
                                                    const companyName = typeof window !== 'undefined' ? localStorage.getItem('companyName') : 'General';

                                                    const shouldFlag = !isFunctionallyCorrect || pasteCount > 2 || tabSwitches > 3 || answer.length < 10 || isAIVoiceDetected;
                                                    let flagReason = '';
                                                    if (!isFunctionallyCorrect) flagReason += 'Failed AI code verification. ';
                                                    if (answer.length < 10) flagReason += 'Insufficient explanation provided. ';
                                                    if (pasteCount > 2) flagReason += `High paste count (${pasteCount}). `;
                                                    if (tabSwitches > 3) flagReason += `Excessive tab switches (${tabSwitches}).`;
                                                    if (isAIVoiceDetected) flagReason += `AI-generated voice detected (${voiceCheckResult?.confidence}% confidence). `;
                                                    ...
                                                    if (studentEmail) {
                                                        fetch('http://localhost:5000/api/exam/submit', {
                                                            ...
                                                            body: JSON.stringify({
                                                                ...
                                                                studentEmail,
                                                                examId: examId || 'HACKATHON_2026',
                                                                companyName: companyName || 'General',
                                                                totalScore: finalScore,
                                                                problems: [{
                                                                    problemId: questionId,
                                                                    score: finalScore,
                                                                    timeTaken: keystrokes  // proxy for time
                                                                }],
                                                                isFlagged: shouldFlag,
                                                                flagReason: shouldFlag ? flagReason.trim() : null
                                                            })
                                                        }).catch(err => console.warn('Could not save exam result:', err));
                                                    }
                                        ...
                                        disabled={isVerifying || isAIVoiceDetected || isCheckingVoice}
                                        className={`flex-1 rounded-xl py-3 flex items-center justify-center gap-2 text-sm font-semibold transition-all shadow-lg ${isVerifying || isCheckingVoice
                                            ? 'bg-blue-500/50 cursor-not-allowed opacity-70 border border-blue-500/20'
                                            : isAIVoiceDetected
                                                ? 'bg-red-500/20 border border-red-500/40 text-red-400 cursor-not-allowed'
                                                : 'premium-gradient hover:opacity-90 shadow-blue-500/10'
                                            }`}
                                    >
                                        {isVerifying ? (
